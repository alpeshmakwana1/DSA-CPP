...
- For arrays with more than one element, iterate through the array and compare each element with the previous one.
...
int main() {
    ...
    for (int i = 1; i < n; i++) {
        //If Previous element is greater than current element array is not sorted
        if (arr[i-1] > arr[i]) {
            sorted = false;
            break;
        }
